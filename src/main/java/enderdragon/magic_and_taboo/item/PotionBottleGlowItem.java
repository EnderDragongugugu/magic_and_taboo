...
public class PotionBottleGlowItem extends MagicPotionItem {
    ...
    public ItemStack finishUsingItem(ItemStack pStack, Level pLevel, LivingEntity pLivingEntity) {
        pStack.getCapability(MATCapabilities.MAGIC_POTION).ifPresent(data -> {
            MagicPotionItem.addEffect(pLevel, getEffectInstances(data, 0.5F, 1), pLivingEntity);
        });
        test(pLivingEntity, pStack, this, MATItems.GLASS_POTION_BOTTLE_GLOW.get());
    ...
    public void appendHoverText(ItemStack pStack, @javax.annotation.Nullable Level pLevel, List<Component> pTooltip, TooltipFlag pFlag) {
        pStack.getCapability(MATCapabilities.MAGIC_POTION).ifPresent(data -> {
            PotionUtils.addPotionTooltip(getEffectInstances(data, 0.5F, 1).stream().toList(), pTooltip, 1.0F);
        });
    }
